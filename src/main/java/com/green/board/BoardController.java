...
/*
    ...
    대용량의 데이터를 보내야할 때에도 body에 데이터를 담아서 보낸다. url은 길이제한이 있기 때문에
    ...
    get방식은 주로 쿼리스트링 방식을 사용하고 - 데이터를 읽어올 때 (간혹 삭제할 때에도 사용함)
    post방식은 body에 데이터를 담아서 보내는 방식을 사용했었다. - 데이터를 저장/수정/삭제할 때
    데이터가 있었을 때에는 get방식이 처리 속도가 빠름, 데이터 처리가 아닌 단순 화면을 띄울 때에도
    get방식을 사용함
    ...
    url은 같은데 mothod(get/post)로 작업을 구분을 했다. (마치 if문 처럼)
    ...
    FE가 BE에게 (url + method + 데이터)를 요청(Request)하고 BE는 JSON으로 응답(Response)
